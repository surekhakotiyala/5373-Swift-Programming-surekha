...
class ViewController: UIViewController , ColorTableViewControllerDelegate{
    ...
    @IBOutlet weak var Rgbcolor: UITextField!
    @IBOutlet weak var Hslcolor: UITextField!
    @IBOutlet weak var Hsvcolor: UITextField!
    //@IBOutlet weak var redSlider: UISlider!
    //@IBOutlet weak var greenSlider: UISlider!
    //@IBOutlet weak var blueSlider: UISlider!
    @IBOutlet weak var historyrecent: UILabel!
    
    @IBOutlet weak var history: UILabel!
    
    @IBOutlet weak var historylast: UILabel!
    ...
    func ColorTableViewChooseColor(controller: ColorTableViewController, chosenColor color: String){
        ...
        var image = UIImage(named: "tsp.png")
        
        if(historylast.backgroundColor == nil && history.backgroundColor == nil){
            historyrecent.backgroundColor = labelColor
        }
        else if(historylast.backgroundColor == nil && history.backgroundColor != nil)
        {
            history.backgroundColor = historyrecent.backgroundColor
            historyrecent.backgroundColor = labelColor
        }
        else{
            historylast.backgroundColor = history.backgroundColor
            history.backgroundColor = historyrecent.backgroundColor
            historyrecent.backgroundColor = labelColor
                }
        //for rgb values
        ...
        let h1 = Float((60*hsl.H)/60)
        let s1 = Float(100*hsl.S)
        let l = Float(100*hsl.L)
        
        //for hsv values
        let hsv = myColors.fetchHSV(color)
        let h2 = Float(hsv.H)
        let s2 = Float(102*hsv.S)
        let v = Float(102*hsv.V)
        var convertRgbcolorToString: Float
        Rgbcolor.text = "\(r)\t\(g)\t\(b)\t"
        Hslcolor.text = "\(h1)\t\(s1)\t\(l)\t"
        Hsvcolor.text = "\(h2)\t\(s2)\t\(v)\t"
